...
const game = {
     ...
     */
    start() {
        if (this.status !== GAME_STATUS_STOPPED) {
            return
        };
        ...
        let timerId = setTimeout(function moveSnake() {
            game.move();
            if (game.status != GAME_STATUS_STOPPED) {
                timerId = setTimeout(moveSnake, 1 / (game.speedSnake));
            };
        }, 1 / (game.speedSnake));
    },
    ...
    move() {
        ...
        /* запрашиваем координаты следующей позиции */

        const nextPosition = snake.getNextPosition();

        /* проверяем не врезалась ли змейка сама в себя */

        snakeOnSnake = snake.parts.findIndex((item) => item.top === nextPosition.top && item.left === nextPosition.left) === -1 ? false : true;
        if (snakeOnSnake) {
            game.stop();
            return
        }
        ...
        if (foundFood !== -1) {
            ...
            /*Изменяем счет */
            game.score += config.foodCost;
            game.renderScore();

            /* Увеличиваем скорость */
            game.speedSnake *= config.speedCoeff;
    ...
    renderScore() {
        const score = this.getScoreElement();
...
const food = {
    ...
    generateItem() {
        ...
        while (foodOnSnake) {
            ...
            foodOnSnake = snake.parts.findIndex((item) => item.top === newItem.top && item.left === newItem.left) === -1 ? false : true;
            if (!foodOnSnake) {
...
function init() {
    ...
    window.addEventListener('keydown', game.changeSnakeDirection);
}
